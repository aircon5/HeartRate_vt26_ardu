...
void checkpulseNInterval(int valueToAnalyse) {

  if (valueToAnalyse > peak) {
    peak = valueToAnalyse;
  } 
  peak *= 0.99;
  ...
  /*if(max_threshold <= 250 && min_threshold <= 250) 
  {
    max_threshold = 300;
    min_threshold = 300;
  }*/
  ...
  if (valueToAnalyse >= (int)max_threshold) {
    //tänd led
    if(at_top == false) {
      at_top = true;
      pulses++;
  ...
  if (valueToAnalyse <= (int)min_threshold) {
    //släck led
    at_top = false;
  }
...
void loop() {
  int scaled_value = normalized_value; 
  ...
  int upper_bound = 700;                   //Maximum signal values
  //int lower_bound = -200;                      //minimum signal values
  ...
  if(valueAvailable) {
    ...
    checkpulseNInterval(normalized_value);
